...
func CreateEmptyRawTransactionAndHash(fromUserID, to, appID, memo string, amount, fee, validHeight int64, txType byte) (string, string, error) {
	if txType == TxType_COMMON {
		...
		txUcoinTransfer, err := NewUcoinTransferTx(fromUserID, to, appID, memo, validHeight, fee, amount)
		if err != nil {
			return "", "", err
		}
