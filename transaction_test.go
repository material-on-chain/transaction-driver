...
func Test_UcoinTransfer(t *testing.T) {
	...
	memo := "test"
	// 构建交易单和哈希
	emptyTrans, hash, err := CreateEmptyRawTransactionAndHash(fromUserID, ToAddress, coin, memo, amount, fee, validHeight, TxType_UcoinTransfer)
	if err != nil {
		t.Error(err)
	} else {
		fmt.Println("empty transaction:\n", emptyTrans)
